...
def calculateAllScores(newreflist, newcandlist):
    ...
    total_calls = sum(len(newcandlist[i]) * len(newreflist[i]) for i in range(len(newcandlist)))

    pbar = tqdm(total=total_calls, desc="evaluaterefcand calls")
...
def calculateSystemScore(totalsemevallist, totalsemevallistpertag, newreflist, newcandlist):
    ...
    for idx, candidate in tqdm(list(enumerate(newcandlist)), total=len(newcandlist), desc="SystemScore entries"):
        if len(newcandlist[idx]) > len(newreflist[idx]):
        ...
        total_combis = len(choosecands) * len(choosescore)

        for combination in tqdm(
            itertools.product(choosecands, choosescore),
            total=total_combis,
            desc=f"Entry {idx+1}/{len(newcandlist)} combis",
            leave=False,       # 바깥 tqdm(SystemScore entries) 깔끔하게 유지
            mininterval=0.3,   # 너무 잦은 redraw 방지 (원하면 0.1로)
        ):
            combiscore = 0
...
if __name__ == "__main__":
    ...
    parser.add_argument("--kg_output")
    parser.add_argument("--reference")
    parser.add_argument("--max_length_diff", default=None)
    args = parser.parse_args()
    result_xml_path, gold_xml_path = convert_to_xml(args.kg_output, args.reference, args.max_length_diff)
